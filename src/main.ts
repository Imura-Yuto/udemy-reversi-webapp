...
app.get('/api/hello', async (requestAnimationFrame, res) =>{
    res.json({
...
app.post('/api/games', async (req, res) => {
    ...
    try {
        ...
        const turnInsertResult = await conn.execute<mysql.ResultSetHeader>(
            'insert turns (game_id, turn_count, next_disc, end_at) values (?, ?, ?, ?)',
            [gameId, 0, DARK, now]
        )
        const turnId = turnInsertResult[0].insertId

        const squareCount = INNITIAL_BOARD.map((line) => line.length).reduce(
            (v1, v2) => v1 + v2,
            0
        )

        const squaresInsertSql = 'insert into squares (turn_id, x, y, disc) values '+
            Array.from(Array(squareCount))
                .map(() => '(?, ?, ?, ?)')
                .join(', ')
            
        const squaresInsertValues: any [] = []
        INNITIAL_BOARD.forEach((line, y) => {
            line.forEach((disc, x) => {
...
app.get('/api/games/latest/turns/:turnCount', async (req, res) => {
    ...
    try {
        ...
        const gameSelectResult = await conn.execute<mysql.RowDataPacket[]>(
            'select id, started_at from games order by id desc limit 1' // 最新の対戦を取得
        )
        ...
        const responseBody = {
            ...
            //TODO 決着がついている場合、game_resuultsテーブルから取得する
            winnerDisc: null
        }
